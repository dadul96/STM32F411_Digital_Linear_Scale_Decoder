...
char input_buff[CDC_DATA_PACKET_SIZE] = {0};
...
static void cdcacm_data_rx_callback(usbd_device *usbd_dev, uint8_t ep)
{
	(void)ep;

	input_buff_len = usbd_ep_read_packet(usbd_dev, 0x01, input_buff, CDC_DATA_PACKET_SIZE);
}
...
static void cdcacm_set_config_callback(usbd_device *usbd_dev, uint16_t wValue)
{
	...
	usbd_ep_setup(usbd_dev, 0x01, USB_ENDPOINT_ATTR_BULK, CDC_DATA_PACKET_SIZE, cdcacm_data_rx_callback);
